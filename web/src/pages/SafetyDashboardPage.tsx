...
// Gercek backend sekli (/safety/dashboard)
interface SafetyDashboard {
  totalActiveDrivers: number
  driversOk: number
  driversWarning: number
  driversCritical: number
  driversBlocked: number
  fleetAverageSafetyScore: number
  fleetAverageHoursWorked: number
...
export default function SafetyDashboardPage() {
  ...
  // /safety/alerts DIZI donuyor; dashboard.activeAlerts de ayni veri.
  const alerts: SafetyAlert[] = Array.isArray(alertsData)
    ? alertsData
    : ((alertsData as { items?: SafetyAlert[] })?.items || dashboardData?.activeAlerts || [])

  const atRisk = (dashboardData?.driversWarning ?? 0) + (dashboardData?.driversCritical ?? 0) + (dashboardData?.driversBlocked ?? 0)
  const totalDrivers = dashboardData?.totalActiveDrivers ?? 0
  const safeDrivers = Math.max(0, totalDrivers - atRisk)
  const avgScore = Math.round(dashboardData?.fleetAverageSafetyScore ?? 0)
  const fatigueCount = alerts.filter(a => (a.alertType || '').toLowerCase().includes('fatigue') || (a.alertType || '').toLowerCase().includes('yorgun')).length

  const kpis = [
    { label: 'Toplam Sürücü', value: totalDrivers.toString(), icon: Shield, color: 'text-blue-600 bg-blue-50' },
    { label: 'Güvenli Sürücü', value: safeDrivers.toString(), icon: Shield, color: 'text-green-600 bg-green-50' },
    { label: 'Risk Altinda', value: atRisk.toString(), icon: AlertTriangle, color: 'text-red-600 bg-red-50' },
    { label: 'Ort. Güvenlik Skoru', value: dashboardData ? `${avgScore}/100` : '—', icon: Shield, color: 'text-purple-600 bg-purple-50' },
  ]
            ...
            <div className="bg-white rounded-2xl border border-slate-200/60 shadow-sm p-6">
              ...
              <div className="space-y-3">
                <div className="flex items-center justify-between p-3 bg-red-50 rounded-xl border border-red-200/60">
                  <span className="text-[13px] font-medium text-red-800">Aktif Uyarı</span>
                  <span className="text-[18px] font-bold text-red-700">{alerts.length}</span>
                </div>
                <div className="flex items-center justify-between p-3 bg-amber-50 rounded-xl border border-amber-200/60">
                  ...
                  <span className="text-[18px] font-bold text-amber-700">{fatigueCount}</span>
                </div>
                <div className="flex items-center justify-between p-3 bg-blue-50 rounded-xl border border-blue-200/60">
                  <span className="text-[13px] font-medium text-blue-800">Ort. Çalışma Saati</span>
                  <span className="text-[18px] font-bold text-blue-700">{(dashboardData?.fleetAverageHoursWorked ?? 0).toFixed(1)} saat</span>
                </div>
              </div>
            </div>
              ...
              <div className="flex items-center justify-center py-4">
                <div className="relative w-32 h-32">
                  <svg className="w-full h-full -rotate-90" viewBox="0 0 36 36">
                    ...
                    <path d="M18 2.0845 a 15.9155 15.9155 0 0 1 0 31.831 a 15.9155 15.9155 0 0 1 0 -31.831" fill="none" stroke={avgScore >= 70 ? '#22c55e' : '#f59e0b'} strokeWidth="3" strokeDasharray={`${avgScore}, 100`} />
                  </svg>
                  <div className="absolute inset-0 flex items-center justify-center">
                    <span className="text-[24px] font-bold text-slate-900">{avgScore}</span>
                  </div>
                </div>
              </div>
            </div>
