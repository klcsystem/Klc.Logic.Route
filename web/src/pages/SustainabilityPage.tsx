...
export default function SustainabilityPage() {
  ...
  const kpis = [
    { label: 'Toplam Emisyon', value: carbonData ? `${carbonData.totalEmissionsKg?.toLocaleString()} kg` : '—', icon: Leaf, color: 'text-red-600 bg-red-50' },
    { label: 'Emisyon/km', value: carbonData ? `${carbonData.emissionsPerKm?.toFixed(2)} kg` : '—', icon: Leaf, color: 'text-orange-600 bg-orange-50' },
    { label: 'Toplam Tasarruf', value: savingsData ? `${savingsData.totalSavingsKg?.toLocaleString()} kg` : '—', icon: Leaf, color: 'text-green-600 bg-green-50' },
    { label: 'ESG Skoru', value: savingsData ? `${savingsData.esgScore}/100` : '—', icon: Leaf, color: 'text-blue-600 bg-blue-50' },
  ]

  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-[22px] font-bold text-slate-900 tracking-tight">Surdurulebilirlik</h1>
        ...
        <>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <div className="bg-white rounded-2xl border border-slate-200/60 shadow-sm p-6">
              <h3 className="text-[15px] font-semibold text-slate-800 mb-4">Aylik Emisyon Trendi</h3>
                ...
                <div className="space-y-3">
                  {carbonData.monthlyTrend.map(item => (
                    <div key={item.month} className="flex items-center gap-3">
                      ...
                      <span className="text-[12px] font-medium text-slate-700 w-24 text-right">{item.emissions?.toLocaleString()} kg</span>
                    </div>
                  ))}
                </div>
              ) : (
            ...
            {/* Tasarruf Detay */}
            <div className="bg-white rounded-2xl border border-slate-200/60 shadow-sm p-6">
              <h3 className="text-[15px] font-semibold text-slate-800 mb-4">Tasarruf Detaylari</h3>
              {savingsData ? (
                <div className="space-y-4">
                  <div className="flex items-center justify-between p-4 bg-green-50 rounded-xl border border-green-200/60">
                    ...
                    <span className="text-[15px] font-bold text-green-700">{savingsData.routeOptimizationSavings?.toLocaleString()} kg CO2</span>
                  </div>
                  <div className="flex items-center justify-between p-4 bg-blue-50 rounded-xl border border-blue-200/60">
                    ...
                    <span className="text-[15px] font-bold text-blue-700">{savingsData.vehicleConsolidationSavings?.toLocaleString()} kg CO2</span>
                  </div>
          ...
          {carbonData?.vehicleBreakdown && carbonData.vehicleBreakdown.length > 0 && (
            <div className="bg-white rounded-2xl border border-slate-200/60 shadow-sm p-6">
              <h3 className="text-[15px] font-semibold text-slate-800 mb-4">Arac Tipi Bazinda Emisyon</h3>
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                {carbonData.vehicleBreakdown.map(item => (
                  <div key={item.vehicleType} className="p-4 bg-slate-50 rounded-xl border border-slate-200/60">
                    ...
                    <div className="text-[20px] font-bold text-slate-900 mt-1">{item.emissions?.toLocaleString()} kg</div>
                    <div className="text-[12px] text-slate-400 mt-0.5">{item.percentage}% toplam</div>
                  </div>
                ))}
              </div>
            </div>
