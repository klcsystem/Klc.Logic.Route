...
import { ArrowLeft, ArrowRight, Package, Weight, Box, Scale, AlertTriangle, Snowflake, Calculator, Check, X, Send, MapPin, CheckCircle2, Loader2 } from 'lucide-react'
...
const STATUS_STEPS: ShipmentStatus[] = ['Draft', 'Calculated', 'Approved', 'Loading', 'InTransit', 'Delivered']

function StatusStepper({ current, labels }: { current: ShipmentStatus; labels: Record<string, string> }) {
        ...
        return (
          <div key={step} className="flex items-center gap-1">
            <div className={`flex items-center gap-1.5 px-3 py-1.5 rounded-full text-[11px] font-medium whitespace-nowrap ${
              isCompleted ? 'bg-green-100 text-green-700' : isCurrent ? 'bg-orange-100 text-orange-700 ring-2 ring-orange-300' : 'bg-slate-100 text-slate-400'
            }`}>
              {isCompleted && <CheckCircle2 className="w-3 h-3" />}
              {labels[step] || step}
            </div>
            {i < STATUS_STEPS.length - 1 && <div className={`w-4 h-0.5 ${isCompleted ? 'bg-green-300' : 'bg-slate-200'}`} />}
          </div>
        )
      })}
    </div>
...
function ActionButtons({ status, shipmentId, t, onAction }: { status: ShipmentStatus; shipmentId: string; t: ReturnType<typeof useI18n>['t']; onAction: () => void }) {
  ...
  const handleCalculate = () => { shipmentsApi.calculate(shipmentId).then(() => onAction()).catch(() => {}) }
  const handleApprove = () => { shipmentsApi.approve(shipmentId).then(() => onAction()).catch(() => {}) }
  const handleSend = () => { shipmentsApi.sendToProvider(shipmentId).then(() => onAction()).catch(() => {}) }
  const handleCancel = () => { shipmentsApi.cancel(shipmentId).then(() => onAction()).catch(() => {}) }

  return (
    <div className="flex items-center gap-3 flex-wrap">
      {status === 'Draft' && <button onClick={handleCalculate} className={btnPrimary}><Calculator className="w-4 h-4" /> {t.shipments.calculate}</button>}
      {status === 'Calculated' && <><button onClick={handleApprove} className={btnPrimary}><Check className="w-4 h-4" /> {t.shipments.approve}</button><button className={btnSecondary}>{t.routeOptimization.selectCarrier}</button></>}
      {status === 'PendingApproval' && <><button onClick={handleApprove} className={btnPrimary}><Check className="w-4 h-4" /> {t.shipments.approve}</button><button onClick={handleCancel} className={btnDanger}><X className="w-4 h-4" /> {t.shipments.reject}</button></>}
      {status === 'Approved' && <button onClick={handleSend} className={btnPrimary}><Send className="w-4 h-4" /> {t.shipments.sendToProviderBtn}</button>}
      ...
      {!['Completed', 'Cancelled'].includes(status) && <button onClick={handleCancel} className={btnDanger}><X className="w-4 h-4" /> {t.shipments.cancelShipment}</button>}
    </div>
  )
}
...
export default function ShipmentDetailPage() {
  ...
  const { data: shipment, isLoading, refetch } = useApi(
  ...
  if (!shipment) {
    return (
      <div className="text-center py-32">
        ...
        <button onClick={() => navigate('/shipments')} className="mt-4 text-[13px] text-orange-500 hover:underline">{t.common.back}</button>
      </div>
    )
  }
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          ...
          <div>
            <div className="flex items-center gap-3">
              ...
              <Badge variant={priorityVariant[shipment.priority] || 'default'}>{shipment.priority}</Badge>
        ...
        <ActionButtons status={shipment.status} shipmentId={shipment.id} t={t} onAction={refetch} />
      </div>

      {/* Status Stepper */}
      <StatusStepper current={shipment.status} labels={statusLabels} />

      {/* Cargo Info Cards */}
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        {[
          { label: t.shipments.totalWeight, value: `${(shipment.totalWeightKg || 0).toLocaleString()} kg`, icon: Weight, color: 'text-blue-600 bg-blue-50' },
          { label: t.shipments.totalVolume, value: `${shipment.totalVolumeM3 || 0} m3`, icon: Box, color: 'text-green-600 bg-green-50' },
          { label: t.shipments.desiWeight, value: `${shipment.totalDesiWeight || 0}`, icon: Package, color: 'text-amber-600 bg-amber-50' },
          { label: t.shipments.chargeableWeight, value: `${shipment.chargeableWeight || 0} kg`, icon: Scale, color: 'text-purple-600 bg-purple-50' },
        ].map((card) => (
      ...
      <div className="grid lg:grid-cols-3 gap-6">
          ...
          {items.length > 0 && (
            <div className="bg-white rounded-2xl border border-slate-200/60 shadow-sm">
              <div className="px-6 py-4 border-b border-slate-100">
                <h2 className="text-[15px] font-semibold text-slate-800">{t.shipments.items} ({items.length})</h2>
              </div>
                  ...
                  <tbody>
                    {items.map((item) => (
                      <tr key={item.id} className="border-b border-slate-50">
                        ...
                        <td className="px-6 py-3 text-right text-[13px] text-slate-600">{item.volumeM3} m3</td>
        ...
        <div className="space-y-6">
          ...
          {/* Driver Info */}
          {((shipment as any).driverName || (shipment as any).vehiclePlate) && (
            <div className="bg-white rounded-2xl border border-slate-200/60 shadow-sm p-6">
              <h3 className="text-[15px] font-semibold text-slate-800 mb-4">{t.shipments.driver}</h3>
              <div className="space-y-3">
                {(shipment as any).driverName && (
                  <div className="flex items-center justify-between">
                    <span className="text-[13px] text-slate-500">{t.shipments.driver}</span>
                    <span className="text-[13px] font-medium text-slate-800">{(shipment as any).driverName}</span>
                  </div>
                )}
                {(shipment as any).vehiclePlate && (
                  <div className="flex items-center justify-between">
                    <span className="text-[13px] text-slate-500">{t.shipments.vehicle}</span>
                    <span className="text-[13px] font-medium text-slate-800">{(shipment as any).vehiclePlate}</span>
                  </div>
                )}
              </div>
            </div>
                ...
                {events.map((ev, i) => (
                  <div key={ev.id} className="flex gap-3">
                    <div className="flex flex-col items-center">
                      <div className={`w-3 h-3 rounded-full mt-1 ${i === events.length - 1 ? 'bg-orange-400 ring-4 ring-orange-100' : 'bg-green-400'}`} />
